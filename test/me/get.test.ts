...
describe('v0: Me: can get me data', () => {
  ...
  it('Charles\'s Me is instantiable', async () => {
    ...
    expect(meData?.staff.preferences?.message_template_favorites).toStrictEqual([{
      id: 'MOCK_TEMP_1'
    }, {
      id: 'MOCK_TEMP_2'
    }])

    expect(meData?.preferences?.message_template_favorites?.length).toBe(2)
    expect(meData?.preferences?.message_template_favorites).toStrictEqual([{
      id: 'MOCK_TEMP_1'
    }, {
      id: 'MOCK_TEMP_2'
    }])
