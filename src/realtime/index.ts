...
export class RealtimeClient extends events.EventEmitter {
  ...
  private handleMessagePayload(topic: string, message: Buffer, packet: Packet): RealtimeMessage {
    const base = {
  ...
  public subscribe(topic: string, cb?: Function): RealtimeClient {
    this.getClient().subscribe(topic, cb as ClientSubscribeCallback)
    return this
  }

  public unsubscribe(topic: string): RealtimeClient {
