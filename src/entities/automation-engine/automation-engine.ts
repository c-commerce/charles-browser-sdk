...
export class AutomationEngine extends UniverseEntity<AutomationEnginePayload, AutomationEngineRawPayload> {
  ...
  public async fetchReasoningTrace (triggerMessageId: string): Promise<ReasoningTraceResponse> {
    if (this.id === null || this.id === undefined) throw new TypeError('AutomationEngine fetchReasoningTrace requires id to be set.')

    try {
      const opts = {
        method: 'GET',
        url: `${this.universe.universeBase}/${this.endpoint}/${this.id}/reasoning-trace/${triggerMessageId}/explain`,
        headers: {
          'Content-Type': 'application/json; charset=utf-8'
        },
        responseType: 'json'
